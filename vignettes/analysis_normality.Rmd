...
First, we calculate the normality across countries and topics for the five categories with:

```{r, message = FALSE, warning = FALSE}
topic_categories <- list(general, specific, methods, budget, theme)
distances <- lapply(topic_categories, function(category){
  probs <- reduce_docs_for_JSd(category) # PARAMETER HERE, CHANGE THIS TO A LOOP
...
```{r}
ggplotly(
  normality_plots[[4]] + ylim(0.5,0.93) + xlim(0.26,0.67) + coord_fixed() + labs(title = "Components of water budget"),
  tooltip = c('topic')
  )
```
